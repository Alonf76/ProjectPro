/* PROJECTPRO MANAGEMENT SUITE 
   Version: 12.8 - Fix: Title Input Focus & Scroll Jumping
*/
...
const Dashboard = () => {
  ...
  // --- ה-FIX המרכזי: רכיב פתק מופרד למניעת רינדור יתר ---
  const NoteCard = React.memo(({ note, level = 0 }) => {
    const [localTitle, setLocalTitle] = useState(note.title);

    // עדכון מקומי מהיר ועדכון ה-State הראשי רק בסיום (onBlur)
    ...
    return (
      <div className={`space-y-4 ${level > 0 ? 'ml-8 mt-4 border-l-2 border-indigo-400/30 pl-6 relative' : ''}`}>
        ...
        <div className={`${theme.card} p-6 rounded-[2.5rem] border-2 group relative transition-all ${note.isClosed ? 'opacity-30 grayscale' : ''}`} 
          ...
          <div className="min-h-[80px] text-sm outline-none font-medium prose prose-sm max-w-none" 
  ...
  return (
    <div className={`flex h-screen ${theme.bg} ${theme.text} transition-all duration-700 font-sans overflow-hidden relative`} dir="ltr">
      ...
      <aside className={`w-64 ${theme.sidebar} p-6 flex flex-col shrink-0 border-r z-50`}>
        ...
        <div className="mb-8"><span className="bg-indigo-600/20 text-indigo-500 text-[10px] font-black px-3 py-1 rounded-full border border-indigo-500/30 uppercase tracking-widest">v12.8 Stable</span></div>
      ...
      <main className="flex-1 overflow-y-auto p-10 relative z-10">
        ...
        {activeTab === 'tasks' && (
           <div className="space-y-6">
             <div className="flex justify-between items-center"><h2 className="text-3xl font-black italic uppercase text-indigo-600">Tasks</h2><button onClick={() => setData([...data, {id: Date.now(), project: "New", task: "New Task", start: new Date().toISOString().split('T')[0], end: new Date().toISOString().split('T')[0], progress: 0, color: "#6366f1"}])} className="bg-indigo-600 text-white px-8 py-3 rounded-2xl font-black text-xs uppercase shadow-xl hover:bg-indigo-700 transition-all flex items-center gap-2"><Plus size={18}/> Add Task</button></div>
             <div className={`${theme.card} rounded-[2.5rem] border overflow-hidden shadow-2xl`}><table className="w-full text-left text-xs"><thead className="bg-white/10 border-b border-white/10 font-black uppercase text-[10px]"><tr><th className="p-5">Project / Task</th><th className="p-5">Dates</th><th className="p-5">Progress</th><th className="p-5 text-center">Action</th></tr></thead><tbody className="divide-y border-white/10 font-bold">{data.map(t => (<tr key={t.id} className="hover:bg-white/5 transition-colors"><td className="p-4 font-black uppercase">{t.task}</td><td className="p-4">{t.start} - {t.end}</td><td className="p-4">{t.progress}%</td><td className="p-4 text-center"><button onClick={() => setData(data.filter(x => x.id !== t.id))} className="text-red-500"><Trash2 size={16}/></button></td></tr>))}</tbody></table></div>
           </div>
        )}
                     ...
                     return Object.values(groups).map((group, idx) => (
                      <React.Fragment key={idx}>
                        ...
                        {group.tasks.map(task => {
                          ...
                          return (
                            <div key={task.id} className={`flex items-center border-b ${theme.border} group transition-colors hover:bg-white/5`}><div className={`w-[200px] shrink-0 p-4 border-r sticky left-0 z-30 flex items-center gap-3 ${isDarkMode ? 'bg-slate-900/90' : 'bg-white/90'} backdrop-blur-lg`}><input type="color" value={task.color || '#6366f1'} readOnly className="w-3 h-3 rounded-full border-none" /><div className="truncate text-[10px] font-black uppercase">{task.task}</div></div><div className="flex-1 h-14 relative"><div className="absolute h-7 top-3.5 rounded-full flex items-center px-3 text-[9px] text-white font-black overflow-hidden shadow-lg" style={{ left: `${left}px`, width: `${Math.max(w, 40)}px`, backgroundColor: task.color || '#6366f1' }}><div className="absolute inset-0 bg-black/20" style={{ width: `${task.progress}%` }} /><span className="relative z-10">{task.progress}%</span></div></div></div>
                          );
                        })}
                      </React.Fragment>
                     ));

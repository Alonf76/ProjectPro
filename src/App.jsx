/* PROJECTPRO MANAGEMENT SUITE 
   Version: 12.7 - Critical Fix: White Screen & Render Guards
*/
...
const Dashboard = () => {
  ...
  const projectRange = useMemo(() => {
    try {
      ...
      let end = new Date(today); end.setDate(today.getDate() + 90);
      if (data && data.length > 0) {
  ...
  const NoteCard = ({ note, level = 0 }) => (
    <div className={`space-y-4 ${level > 0 ? 'ml-8 mt-4 border-l-2 border-indigo-400/30 pl-6 relative' : ''}`}>
      ...
      <div className={`${theme.card} p-6 rounded-[2.5rem] border-2 group relative transition-all ${note.isClosed ? 'opacity-30 grayscale' : ''}`} 
        ...
        <div className="min-h-[80px] text-sm outline-none font-medium prose prose-sm max-w-none" contentEditable suppressContentEditableWarning onBlur={(e) => updateNote(note.id, 'content', e.currentTarget.innerHTML)} dangerouslySetInnerHTML={{ __html: note.content || '' }} style={{ unicodeBidi: 'plaintext', textAlign: 'initial' }} />
        <div className="mt-6 pt-4 border-t border-black/5 flex justify-between items-center">
          <div className="flex items-center gap-2">
             <input type="color" value={note.color || '#fefcbf'} onChange={(e) => updateNote(note.id, 'color', e.target.value)} className="w-5 h-5 rounded-full border-none cursor-pointer" />
          </div>
  ...
  return (
    <div className={`flex h-screen ${theme.bg} ${theme.text} transition-all duration-700 font-sans overflow-hidden relative`} dir="ltr">
      ...
      <aside className={`w-64 ${theme.sidebar} p-6 flex flex-col shrink-0 border-r z-50`}>
        ...
        <div className="mb-8"><span className="bg-indigo-600/20 text-indigo-500 text-[10px] font-black px-3 py-1 rounded-full border border-indigo-500/30 uppercase tracking-widest">v12.7 Stable</span></div>
            ...
            <div className={`${theme.card} rounded-[2.5rem] border overflow-hidden shadow-2xl`}>
              <table className="w-full text-left">
                <thead className={`${isDarkMode ? 'bg-white/5' : 'bg-white/20'} border-b ${theme.border} text-[10px] font-black uppercase tracking-widest`}>
                  <tr><th className="p-5">Project / Task</th><th className="p-5">Dates</th><th className="p-5">Progress</th><th className="p-5 text-center">Action</th></tr>
                </thead>
             ...
             <div className="flex-1 overflow-auto relative" ref={ganttContainerRef}>
               <div style={{ width: `${projectRange.totalDays * 40 * zoomScale}px` }} className="relative min-h-full">
                 ...
                 <div className="relative z-10">
                   {(() => {
                     const groups = {}; 
                     data.forEach(task => { if (!groups[task.project]) groups[task.project] = { name: task.project, tasks: [] }; groups[task.project].tasks.push(task); });
                     return Object.values(groups).map((group, idx) => (
                      <React.Fragment key={idx}>
                        <div className={`flex items-center border-b ${theme.border} bg-indigo-500/5 sticky left-0 z-20`}>
                           <div className={`w-[200px] shrink-0 p-4 border-r sticky left-0 z-30 font-black text-[10px] text-indigo-500 uppercase ${isDarkMode ? 'bg-slate-900/90' : 'bg-white/90'} backdrop-blur-lg`}>📁 {group.name}</div>
                          ...
                          return (
                            <div key={task.id} className={`flex items-center border-b ${theme.border} group transition-colors hover:bg-white/5`}>
                              <div className={`w-[200px] shrink-0 p-4 border-r sticky left-0 z-30 flex items-center gap-3 ${isDarkMode ? 'bg-slate-900/90' : 'bg-white/90'} backdrop-blur-lg`}>
                                 <input type="color" value={task.color || '#6366f1'} readOnly className="w-3 h-3 rounded-full border-none" />
                                 <div className="truncate text-[10px] font-black uppercase">{task.task}</div>
                              </div>
                              <div className="flex-1 h-14 relative">
                                 <div className="absolute h-7 top-3.5 rounded-full flex items-center px-3 text-[9px] text-white font-black overflow-hidden shadow-lg" 
                                      style={{ left: `${left}px`, width: `${Math.max(w, 40)}px`, backgroundColor: task.color || '#6366f1' }}>
                                 ...
                                 <div className="absolute top-3.5 flex flex-col items-center" style={{ left: `${left + w}px`, transform: 'translateX(-50%)' }}>
                                    <div className="w-3 h-3 rotate-45 border-2 border-white shadow-md" style={{ backgroundColor: task.color || '#6366f1' }} />
        ...
        {activeTab === 'team' && (
           <div className={`max-w-4xl ${theme.card} p-12 rounded-[3rem] border mx-auto`}>
              ...
              <div className="flex gap-4 mb-12">
                <input placeholder="Name" className={`flex-1 p-4 rounded-2xl border ${theme.input}`} value={newMember.name} onChange={e => setNewMember({...newMember, name: e.target.value})} /><button onClick={() => {if(newMember.name) setTeam([...team, newMember]); setNewMember({name:'', role:''})}} className="bg-indigo-600 text-white px-12 py-4 rounded-2xl font-black uppercase text-xs">Add</button>
              </div>
              <div className="grid grid-cols-2 gap-5">{team.map((m, i) => (<div key={i} className="flex justify-between items-center p-6 bg-white/5 border border-white/10 rounded-[2rem]"><div className="flex items-center gap-5"><div className="w-12 h-12 bg-indigo-600 text-white rounded-xl flex items-center justify-center font-black">{m.name.charAt(0)}</div><p className="font-black text-sm uppercase">{m.name}</p></div><button onClick={() => setTeam(team.filter((_, idx) => idx !== i))} className="text-red-500"><Trash2 size={18}/></button></div>))}</div>
           </div>
        )}
      </main>
    </div>
